"""
Climatebase job crawler.

For jobs that link to external platforms (Greenhouse, Lever, Ashby),
uses the existing crawlers to get full details.

For jobs that stay on other platforms (Workday, SmartRecruiters, etc.),
marks as crawled since we can't easily scrape those.
"""
...
def crawl_climatebase_job(job: Job) -> Optional[Job]:
    """
    Crawl and update a Climatebase job.

    Detects the application URL domain and uses the appropriate
    platform crawler to get full details.

    Args:
        job: Job instance from climatebase source
    ...
    # For other URLs (Workday, SmartRecruiters, etc.),
    # just mark as crawled since we can't scrape those
