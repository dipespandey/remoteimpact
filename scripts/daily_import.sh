...
#   0 6 * * * /path/to/remoteimpact/scripts/daily_import.sh >> /var/log/remoteimpact/import.log 2>&1
#
# This script:
...
# 4. Optionally runs AI parsing to enrich job descriptions
#
...
# Activate virtual environment if using one (for non-uv setups)
# source venv/bin/activate

# Step 1: Import from aggregator APIs
echo ""
echo "[Step 1/3] Importing from aggregator APIs..."
if [ "$USE_AI" = "true" ]; then
...
# Step 2: Import job URLs from Google Search
echo ""
echo "[Step 2/3] Importing URLs from Google Search..."
uv run python manage.py import_google_jobs --unified --num-results 100

# Step 3: Crawl newly discovered jobs
echo ""
echo "[Step 3/3] Crawling job details from discovered URLs..."
if [ "$USE_AI" = "true" ]; then
    uv run python manage.py crawl_jobs --limit 200 --use-ai --batch-size 5
else
